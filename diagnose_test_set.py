...
def diagnose_model_outputs(model, dataloader, device, score_threshold=0.05, iou_threshold=0.5):
    """诊断模型输出"""
    model.eval()
    ...
    if len(all_pred_scores) > 0:
        ...
        if len(all_ious) > 0:
            print(f"\nIoU统计 (与GT的最大IoU):")
            print(f"  最小值: {np.min(all_ious):.4f}")
            print(f"  最大值: {np.max(all_ious):.4f}")
            print(f"  平均值: {np.mean(all_ious):.4f}")
            print(f"  中位数: {np.median(all_ious):.4f}")
            print(f"  标准差: {np.std(all_ious):.4f}")
            print(f"  IoU >= {iou_threshold}: {np.sum(np.array(all_ious) >= iou_threshold)} ({np.sum(np.array(all_ious) >= iou_threshold)/len(all_ious)*100:.1f}%)")
            print(f"  IoU >= 0.3: {np.sum(np.array(all_ious) >= 0.3)} ({np.sum(np.array(all_ious) >= 0.3)/len(all_ious)*100:.1f}%)")
            print(f"  IoU >= 0.1: {np.sum(np.array(all_ious) >= 0.1)} ({np.sum(np.array(all_ious) >= 0.1)/len(all_ious)*100:.1f}%)")
    else:
...
def main():
    parser = argparse.ArgumentParser(description='诊断测试集评估问题')
